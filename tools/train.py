...
def main():
    ...
    cfg.work_dir=cfg.work_dir+'/'+cfg.split
    
    cfg.dataset.train.ann_file=cfg.annotation_path
    cfg.dataset.val.ann_file=cfg.annotation_path
    cfg.dataset.test.ann_file=cfg.annotation_path
    cfg.evaluation.ground_truth_filename=cfg.annotation_path
    
    if 'feature_stride' in cfg:
        cfg.dataset.train.feature_stride=cfg.feature_stride
        cfg.dataset.val.feature_stride=cfg.feature_stride
        cfg.dataset.test.feature_stride=cfg.feature_stride
    if 'data_path' in cfg:
        cfg.dataset.train.data_path=cfg.data_path
        cfg.dataset.val.data_path=cfg.data_path
        cfg.dataset.test.data_path=cfg.data_path

    # print(cfg.dataset.train.feature_stride)
    # print(cfg.dataset.val.feature_stride)
    # print(cfg.dataset.test.feature_stride)
    # print(cfg.feature_stride)
    # print(cfg.dataset.train.data_path)
    # print(cfg.dataset.val.data_path)
    # print(cfg.dataset.test.data_path)
    # print(cfg.data_path)
    # exit()

    # info=cfg.annotation_path.split('.')
    # info[1]=cfg.split
    # cfg.annotation_path=cfg.annotation_path+'.{}.json'.format(cfg.split)

    # print(cfg.annotation_path)
    # print(cfg.work_dir)
    # return 
    ...
    if args.resume != None:
        ...
        # best_mAP_path=cfg.work_dir+'/checkpoint/best_mAP.pth'
        # if os.path.exists(best_mAP_path):
        #     checkpoint = torch.load(best_mAP_path, map_location=device)
        #     # best_mAP = checkpoint['best_mAP']
        #     cfg.best_mAP=checkpoint['best_mAP']
        #     del checkpoint
        # else:
        #     cfg.best_mAP=0
    ...
    for epoch in range(resume_epoch + 1, max_epoch):
        ...
        if (epoch == max_epoch - 1) or ((epoch + 1) % cfg.workflow.checkpoint_interval == 0):
            if args.rank == 0:
                ...
                # print("# location 11.1")
                # print(type(cfg.save_checkpoint_cache))
        # val for one epoch
        ...
        if epoch >= val_start_epoch:
            if (cfg.workflow.val_eval_interval > 0) and ((epoch + 1) % cfg.workflow.val_eval_interval == 0):
                # assert cfg.annotation_path.endswith('json'), 'cfg.annotation_path.endswith json'
                # print(cfg.annotation_path)
                eval_one_epoch(
                    test_loader,
                    model,
