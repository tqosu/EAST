...
parser.add_argument('--video_prefix',type=str, default='video1')
...
def main():
    ...
    vid_prefix='./data/{}/{}/'.format(args.dataset,args.video_prefix)
    vid_prefix0='./data/{}/video/'.format(args.dataset)
    ...
    #         # return 
            ...
            content=content[::args.downsample]
            cnt=0
            # print(properties['frame'],len(content))

            # if args.dataset=='50salads':
            if len(content)<properties['frame']:
                cnt+=1
                content.append(content[-1])
            else:
                cnt=properties['frame']-len(content)
                content=content[:properties['frame']]
            assert properties['frame']==len(content),print(properties['frame'],len(content),cnt)
            # print(properties)
            # return
            # continue
            ...
            # print(properties)
    # return     
    filename = path1+args.dataset+".fps{}.split{}".format(args.fps,args.split)+".json"
    with open(filename, 'w') as file:
