...
class mAP:
    def __init__(
        ...
        **kwargs
        ...
        # self.num_classes = kwargs.get("num_classes", len(self.activity_index))
        ...
        self.ground_truth, self.activity_index, self.activity_index_all, self.video_info = self._import_ground_truth(ground_truth_filename)
        self.num_classes=len(self.activity_index_all)
        # print(self.activity_index)
        # exit()
        self.prediction = self._import_prediction(prediction_filename)
        if kwargs.get("save_npz", False):
            self._save_npz()
    ...
    def _save_npz(self):
        ...
        prediction_groups = self.prediction.groupby("video-id")
        ground_truth_groups = self.ground_truth.groupby("video-id")
        for video_id, group_df in prediction_groups:
            rate = self.video_info[video_id]['frame']/self.video_info[video_id]['duration']
            total_frame = self.video_info[video_id]['frame']
            num_classes = self.num_classes
            pred=np.zeros((total_frame, num_classes))
            gt=np.zeros((total_frame), dtype=int)-100

            for idx, this_gt in ground_truth_groups.get_group(video_id).iterrows():
                label,s,e=this_gt[['label',"t-start", "t-end"]].values
                s,e=int(s*rate),int(e*rate)
                gt[s:e]=label

            for idx, this_pred in group_df.iterrows():
                label, s, e, score=this_pred[['label',"t-start", "t-end", 'score']].values
                s,e=int(s*rate),int(e*rate)
                pred[s:e, label]+=score
                # pred[s:e, label]+=score
            pred = normalize_last_dim(pred)
            group_df['t-start']=group_df['t-start']*rate
            group_df['t-end']=group_df['t-end']*rate

            ground_truth1=ground_truth_groups.get_group(video_id)
            # ground_truth1['t-start']=ground_truth1['t-start']*rate
            # ground_truth1['t-end']=ground_truth1['t-end']*rate

            np.savez(self.work_dir+'/'+video_id+'.npz', gt=gt, prob=pred, seg=group_df[['label',"t-start", "t-end", 'score']].values,
                ground_truth=ground_truth1[['label',"t-start", "t-end"]].values, rate=rate)
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        for videoid, v in data["database"].items():
            ...
            video_info[videoid]['duration']=v['duration']
            video_info[videoid]['frame']=v['frame']
        ...
        return ground_truth, activity_index_ret, activity_index, video_info
