...
def save_met2csv(cfg, training_log):
    csv_file_path = cfg.work_dir+ cfg.metr_path

    # Check if the CSV file exists
    file_exists = os.path.isfile(csv_file_path)

    # Get the keys from the first dictionary to use as headers
    headers = training_log.keys()

    # Write the logs to the CSV file
    with open(csv_file_path, mode='a', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=headers)
        if not file_exists:
            writer.writeheader()  # Only write headers if the file doesn't exist
        writer.writerow(training_log)
...
def eval_one_epoch(
    ...
    if rank == 0:
        ...
        cfg.evaluation.work_dir=cfg.work_dir+'evaluation'
        if not not_eval:
            # build evaluator
            ...
            # print('adapter_index1' in cfg.model.backbone.backbone)
            # exit()
            if 'adapter_index1' in cfg.model.backbone.backbone:
                metrics_dict['adapter_index1']=cfg.model.backbone.backbone.adapter_index1
            print(metrics_dict)
            
            save_met2csv(cfg, metrics_dict)
