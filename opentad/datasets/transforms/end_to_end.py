...
class LoadSnippetFrames:
    ...
    def random_trunc(self, feats, trunc_len, gt_segments, gt_labels, offset=0, max_num_trials=200):
        ...
        for _ in range(max_num_trials):
            ...
            seg_idx = (inter >= self.trunc_thresh*area_segs)
...
class LoadFrames:
    ...
    def __call__(self, results):
        ...
        if self.method == "resize":
            ...
            # print(total_frames,frame_stride,self.scale_factor)
            # print(results["gt_segments"])
            # return 
            ...
            if len(frame_idxs) < frame_num:
                valid_len = int(len(frame_idxs) // self.scale_factor)
                # print(frame_idxs)
                # print((0, frame_num - len(frame_idxs)))
                frame_idxs = np.pad(frame_idxs, (0, int(frame_num - len(frame_idxs))), mode="edge")
            ...
            # print(results.keys())
            # print(results)
            # print(type(frame_idxs),frame_idxs.shape)
            # print(frame_idxs)
            # print(total_frames, frame_stride)
            # exit()
            # print('# location 1')
            # print(results.keys())
            # print(results['video_name'])
            # print(results['feature_start_idx'],results['feature_end_idx'],self.scale_factor)
            # print(total_frames,start_idx,end_idx)
            # print(frame_idxs)
