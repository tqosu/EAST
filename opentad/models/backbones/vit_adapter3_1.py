...
class Adapter(BaseModule):
    ...
    def forward(self, x: Tensor, h: int, w: int) -> Tensor:
        ...
        b, t, h, w, c = attn.shape
        # tube dropout
        mask = torch.ones(b, 1, h, w, 1,device=x.device)  # Shape [b, 1, h, w, 1]
        mask = self.attn_drop(mask)
        attn = attn * mask

        attn = torch.mean(attn, dim=(2, 3))
        
        attn = attn.permute(0, 2, 1)  # [b*h*w,c,t] [1*10*10,384,384]
...
class VisionTransformerAdapter3_1(BaseModule):
