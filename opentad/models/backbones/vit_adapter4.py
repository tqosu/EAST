...
class Adapter(BaseModule):
    def __init__(
        ...
        self.attn_drop = nn.Dropout(adapter_drop_rate)
        # print(f"adapter_drop_rate: {adapter_drop_rate}")
        # exit()
    ...
    def forward(self, x: Tensor, h: int, w: int) -> Tensor:
        ...
        x = x.reshape(-1, self.temporal_size, h, w, x.shape[-1])  # [b,t,h,w,c]  [1,384,10,10,384]
        # temporal depth-wise convolution
        x = self.attn_drop(x)
        x = torch.mean(x, dim=(2, 3)) # b t c
        

        # down and up projection
        x = self.down_proj(x)
        x = self.act(x)
        attn = x.permute(0, 2, 1)  # [b*h*w,c,t] [1*10*10,384,384]
        
        attn = self.dwconv(attn)  # [b*h*w,c,t] [1*10*10,384,384]
        attn = self.conv(attn)  # [b*h*w,c,t] [1*10*10,384,384]
        # attn = attn.unflatten(0, (-1, 1, 1)).permute(0, 4, 1, 2, 3)  # [b,t,h,w,c] [1,384,10,10,384]
        # attn = attn.expand(-1, -1, h, w, -1)
        # attn = attn.reshape(B, N, C)
        # print(attn.shape,x.shape)
        attn = attn.permute(0, 2, 1)  # [b, t, c] 
        ...
        x = x.unflatten(0, (-1, 1, 1)).permute(0, 4, 1, 2, 3) # [b,t,h,w,c] [1,384,10,10,384]
        x = x.expand(-1, -1, h, w, -1)
        x = x.reshape(B, N, C)
        ...
        **kwargs,
        ...
        if self.use_adapter:
            self.adapter = Adapter(
                ...
                adapter_drop_rate=adapter_drop_rate,
            )
...
class VisionTransformerAdapter4(BaseModule):
