...
class LadderNetwork(BaseModule):
    ...
    def forward(self, feat_list: List[Tensor], h, w, x) -> Tensor:
...
class VisionTransformerAdapterM1(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.adapter_index2 = adapter_index2
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x, h, w)
            if i in self.adapter_index2:
                # feat_list.append(x)
                feat_list.append(x.detach())

        # print(x.shape)
        x = self.ladder(feat_list, h, w, x)
        # print(x.shape)
        # exit()
        x = self.norm(x)
